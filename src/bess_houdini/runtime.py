...
class Runtime:
    ...
    def connect(self, executable, cwd, storage, allow_changes=True):
        ...
        self.call("initialize", {
            "clientInfo": {"name": "bess_houdini", "title": "Bess", "version": "0.1.0"},
    ...
    def _list_mcp(self, cursor=None):
        self.call("mcpServerStatus/list", {"cursor": cursor, "limit": 100,
                  "detail": "toolsAndAuthOnly"}, self._mcp_listed)

    def _mcp_listed(self, result):
        self.mcp_names.update(x["name"] for x in result.get("data", []))
        if result.get("nextCursor"):
            self._list_mcp(result["nextCursor"])
        else:
            self.ready = True
            self.status = "接続済み"
            self.changed()
    ...
    def new_chat(self):
        ...
        self.resume_needed = False
        self.tokens = ""
        self.resolved_model = self.resolved_effort = ""
        self.changed()
    ...
    def load(self, session_id):
        ...
        cwd = data.get("cwd", self.cwd)
        if not Path(cwd).is_dir():
            raise ValueError("履歴の作業フォルダーが見つかりません。設定を確認してください。")
    ...
    def send(self, prompt, model, effort):
        ...
        def start_turn(_=None):
            ...
            self.call("turn/start", {"threadId": self.thread_id,
                "input": [{"type": "text", "text": prompt}], "model": model, "cwd": self.cwd,
                "effort": effort}, self._turn_started)
            self.resolved_model, self.resolved_effort = model, effort

        if not self.thread_id or self.resume_needed:
            self.prepare_thread(model, start_turn)
        else:
            start_turn()

    def prepare_thread(self, model, callback=None):
        """モデル呼び出しなしでも接続・動的ツールの登録を検証できる。"""
        from .houdini_tools import TOOLS, INSTRUCTIONS
        params = {"model": model, "cwd": self.cwd, "approvalPolicy": "never",
                  ...
                  "config": {"mcp_servers": {n: {"enabled": False} for n in self.mcp_names}},
                  "dynamicTools": TOOLS if self.allow_changes else [x for x in TOOLS if x["name"] != "houdini_python"]}
        method = "thread/start"
        if self.thread_id:
            method = "thread/resume"
            params["threadId"] = self.thread_id

        def verify(result, cursor=None):
            self.call("mcpServerStatus/list", {"threadId": self.thread_id, "cursor": cursor,
                      "limit": 100, "detail": "toolsAndAuthOnly"}, checked)

        def checked(result):
            if any(x.get("tools") for x in result.get("data", [])):
                self.error("この会話に外部MCPツールが残っています。再接続してください。")
                return
            if result.get("nextCursor"):
                verify(None, result["nextCursor"])
            elif callback:
                callback({})

        def started(result):
            self.thread_id = result["thread"]["id"]
            self.resolved_model = result.get("model", model)
            self.resolved_effort = result.get("reasoningEffort") or ""
            self.save()
            verify(result)
        self.call(method, params, started)
    ...
    def steer(self, prompt, accepted=None):
        if not self.busy or not self.turn_id or self.interrupted or self.steering:
        ...
        self.call("turn/steer", {"threadId": self.thread_id,
            "expectedTurnId": self.turn_id, "input": [{"type": "text", "text": prompt}]}, done)
    ...
    def handle(self, event):
        if "id" in event and "method" not in event:
            ...
            if event.get("error"):
                ...
                if method == "turn/steer":
                    self.steering = False
                    self.add("system", "追加指示を送れませんでした: " + message)
                else:
                    self.error(message)
            elif callback:
        ...
        if "id" in event:
            if method == "item/tool/call":
                from .houdini_tools import execute, text_result
                ...
                description = arguments.get("description") or {"houdini_context": "シーンを確認", "houdini_screenshot": "画面を確認"}.get(params["tool"], "Houdini を操作")
                ...
                self.transport.send({"id": event["id"], "error": {"code": -32601, "message": "Bessでは未対応の要求です: " + method}})
            return
        if params.get("threadId") and params["threadId"] != self.thread_id:
            return
        event_turn = params.get("turnId") or (params.get("turn", {}).get("id") if method.startswith("turn/") else None)
        ...
        if method == "item/agentMessage/delta":
            ...
            self.changed()
        elif method == "bess_houdini/disconnected":
            self.ready = self.busy = False
            self.authenticated = self.steering = False
            self.resume_needed = bool(self.thread_id)
            self.requests.clear()
            self.pending.clear()
            self.status = "切断されました。再接続してください"
            if self.transport and self.transport.stderr:
                self.add("system", "Codex起動・通信エラー:\n" + "\n".join(self.transport.stderr)[-6000:])
            self.changed()
            self.save()
